    ...
    for NCtr in range(1,NAt+1):
        ...
        while True:
            ...
            if PP>0:
                ...
                #SortPrel
                IndF = SortPrel(PrelOcc,PrelVec,PrelCtr,PP,NBOOcc,NBOVec,NBOCtr,IndF)
                #-----------------------------
...
def SortPrel(PrelOcc, PrelVec, PrelCtr, PP, NBOOcc,NBOVec,NBOCtr,IndF):
...
def Input():
    ...
    NAOAO = np.zeros((BSz, BSz), dtype=np.float64)
...
def NBOPlotMolden(NBOOcc,NBOVec,NBOAmnt):
    ...
    while Resid>0:
        ...
        fp2.write("{:10.5f}{:10.5f}{:10.5f}{:10.5f}{:10.5f}\n".format(NBOOcc[5*k],NBOOcc[5*k+1],NBOOcc[5*k+2],NBOOcc[5*k+3],NBOOcc[5*k+4]))
        fp2.write("-"*50+'\n')
        for i in range(BSz):
            # dtln = fp1.readline()
            # dtln = dtln[:21]
